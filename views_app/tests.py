...
class AuthorViewTestCase(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.user = User.objects.create_user(username=self.username, password=self.password, email='email1@email.com')
        ...
        self.user_with_permissions = User.objects.create_user(username='user_with_permissions',
                                                              password='permissions', email='email3@email.com')
...
class TestCategories(TestCase):
    def setUp(self) -> None:
        self.author = Author.objects.create(name='test_name', bio='bio', email='test@test.com')
        self.post = Post.objects.create(title='test', content='content', author=self.author)
        self.category = Category.objects.create(name='test_cat', description='test_desc')

    def test_add_category(self):
        self.post.categories.add(self.category)
        self.post.categories.remove(self.category)
        print('Test done!')
